...
const ReportModal = ({ entity, onClose, createReportFn, addToast }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      doc.text('Suspicious Activity Report (SAR)', margin, margin + 5);
      
      // Entity info
      doc.setFontSize(10);
      doc.setFont(undefined, 'normal');
      doc.text(`Account ID: ${nameToDisplay}`, margin, margin + 15);
      doc.text(`Generated: ${new Date().toLocaleString()}`, margin, margin + 22);
      doc.text(`Report Type: ${reportType === 'sar' ? 'Suspicious Activity Report' : 'Compliance Profile'}`, margin, margin + 29);
      ...
      doc.text('This report was generated by Nexara AML Platform', margin, pageHeight - 10);
      
      // Save the PDF
      const filename = `SAR_${nameToDisplay}_${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(filename);
      
      if (addToast) addToast(`PDF downloaded as ${filename}`, 'success');
    } catch (err) {
            ...
            {report && (
              <div className="flex gap-2.5">
                ...
                <button 
                  ...
                  className="px-5 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2.5"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 16v-8m-4 4h8M4 12a8 8 0 118 8 8 8 0 01-8-8z" /></svg>
                  Download PDF
                </button>
              </div>
            )}
